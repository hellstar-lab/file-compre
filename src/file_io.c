/******************************************************************************
 * Advanced File Compressor - File I/O Module
...
static DecompFileType DetectFileTypeFromContent(const uint8_t* data, size_t size) {
    ...
    if (is_text) {
        ...
        if (memmem(data, size, "{", 1) && memmem(data, size, "}", 1)) {
            return FILE_TYPE_JSON;
        }
        if (memmem(data, size, "<", 1) && memmem(data, size, ">", 1)) {
            return FILE_TYPE_XML;
        }
        if (memmem(data, size, ",", 1)) {
    ...
    // Check for PDF
    if ((size >= 7 && memcmp(data, "%PDF-", 5) == 0) || (size >= 4 && memcmp(data, "%PDF", 4) == 0)) {
        return FILE_TYPE_PDF;
    }
    
    // Check for images
    if (size >= 8 && memcmp(data, "\x89PNG", 8) == 0) {
...
// Portable memmem implementation for environments without GNU extension
