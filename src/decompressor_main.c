/******************************************************************************
 * Advanced File Compressor - Main Decompressor Module
...
// Portable case-insensitive string comparison to avoid platform differences
    ...
    snprintf(search_pattern, sizeof(search_pattern), "%s/*.comp", input_dir);
        ...
        do {
            if (!(find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                ...
                strncpy(base_name, find_data.cFileName, DECOMP_MAX_FILENAME - 1);
                base_name[DECOMP_MAX_FILENAME - 1] = '\0';
