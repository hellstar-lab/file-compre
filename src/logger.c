/******************************************************************************
 * Advanced File Compressor - Logger Module
...
int Logger_Init(const Logger* logger_config) {
    if (logger_config) {
        ...
        // `log_file` is a fixed-size char array in the config; the address check is
        // always true. Only check for non-empty content to avoid -Waddress.
        if (strlen(logger_config->log_file) > 0) {
...
void Logger_LogFileComplete(const char* filename, DecompStatus status, 
    ...
    (void)filename; // unused parameter (filename is included in log context if needed)
    const char* status_desc = Utility_GetStatusDescription(status);
...
static const char* GetLevelColor(LogLevel level) {
    switch (level) {
        ...
        case LOG_LEVEL_TRACE:   return "\033[35m"; // Magenta
...
static void WriteToConsole(const char* message, LogLevel level) {
        ...
        SetConsoleTextAttribute(hConsole, saved_attributes);
    #else
        // Unix-like console output with colors
...
static DecompStatus CreateLogDirectory(const char* directory) {
        ...
        char temp_path[DECOMP_MAX_PATH];
        strncpy(temp_path, directory, DECOMP_MAX_PATH - 1);
        temp_path[DECOMP_MAX_PATH - 1] = '\0';
